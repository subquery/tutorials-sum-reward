...
type StakingSlashProps = Omit<StakingSlash, NonNullable<FunctionPropertyNames<StakingSlash>>>;
...
export class StakingSlash implements Entity {
    ...
    static async get(id:string): Promise<StakingSlash | undefined>{
        ...
        if (record){
            return StakingSlash.create(record as StakingSlashProps);
        }else{
            return;
        }
    }
    ...
    static async getByAccountId(accountId: string): Promise<StakingSlash[] | undefined>{
      ...
      return records.map(record => StakingSlash.create(record as StakingSlashProps));
      
    }


    static create(record: StakingSlashProps): StakingSlash {
